...
class MyOptionMenu(OptionMenu):
    def __init__(self, master, status, *options):
        ...
        self.config(font=('calibri',(25)),bg='white',width=25, height=3)
        self['menu'].config(font=('calibri',(25)),bg='white')


root = Tk()
username = MyOptionMenu(root, u'Select User Name \u25BC', *users)
location = MyOptionMenu(root, u'Select Location \u25BC', *places)
username.pack()
location.pack()

def ok():
        FileInfo=(username.var).get()+'_'+(location.var).get()
        root.destroy()
        print((username.var).get()+ ' is collecting data at '+(location.var).get())
        populateData(FileInfo)

button = Button(root, text="Start Data Collection", command=ok)
button.config(width=27, height=3, bg='white', font= 'Calibri 25')
button.pack()
