			...
			if n>1:
				...
				if f[6]=='TR':
					if  not int(f[1]) in vehNr:
						...
						vehDel.append([0, 0, 0, 0])
					if int(f[5])==0:
						vehDel[vehNr.index(int(f[1]))][int(f[4])]+=1		

	vehDel=list(map(list, zip(*vehDel)))

	for i in range(len(vehDel)):
		vehDel[i]=sum(vehDel[i])/len(vehDel[i])
	delays.append(vehDel)

print(list(map(list, zip(*delays))))